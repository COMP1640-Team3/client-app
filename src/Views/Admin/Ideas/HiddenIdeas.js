...
const HidenIdeas = () => {
    ...
    const getIdeas = async (pageNumber = 1) => {
        try {
            if (query == null) {
                const response = await Api().get(`/admins/hidden-ideas?page=${pageNumber}`);
                setIdeas(response.data);
            } else {
                if (query.length === 0) {
                    const response = await Api().get(`/admins/hidden-ideas/?page=1`);
                    setIdeas(response.data);
                }
                const response = await Api().get(`/admins/hidden-ideas?page=${pageNumber}&title=${query}`);
                setIdeas(response.data);
            }
        } catch (error) {
            console.log(error);
    ...
    return (<>
        ...
        <Stack mb='5'>
            <Box>
                <Input onChange={changeTitle} placeholder='Enter title to search hidden ideas in system 💡' type='text'
